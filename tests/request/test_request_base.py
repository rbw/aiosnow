...
async def test_core_unexpected_response_content(mock_client):
    """Non-JSON data in content should raise an UnexpectedResponseContent"""

    session = await mock_client(server_path="/test")
...
async def test_core_error_full(mock_client, mock_error):
...
async def test_core_error_message_only(mock_client, mock_error):
    """HTTP error response with null detail should work"""
...
async def test_core_error_fallback_500(mock_client):
...
async def test_core_response_204(mock_client):
...
async def test_core_response_malformed(mock_client):
