...
class StreamGroup:
    ...
    def targets(self):
        for c in range(self.count):
            print(c * (self.size + self._position))
    ...
    async def execute(self):
        ...
        self._position = self.count * (self.size + self._position)
...
class Session:
    ...
    def __init__(self, path):
        ...
        self.group = StreamGroup(session=self, size=10, count=5)
...
async def get_many():
    async with Session("/api/now/table/incident") as session:
        async for record in session.pump():
            pass # print(record)
